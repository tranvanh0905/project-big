...
@section('script')
    <script>
        //Tải bài hát gợi ý cho trang chi tiết playlist cá nhân
        let playlistId = $('.album-wrap').attr('data-playlist-id');
        $.ajax({
            type: "GET",
            url: "/user/library/user-playlist/single-playlist/" + playlistId + "/suggest-song",
            success: function (data) {
                let getdata = $(data)
                $('#suggest-song').fadeOut().append(getdata).fadeIn();
            }
        });

        $('#reload-suggest').on('click', function () {
            $.ajax({
                type: "GET",
                url: "{{route('suggestSongUserPlaylist', ['playlistId' => $singlePlaylist->id])}}",
                success: function (data) {
                    let getdata = $(data)
